...
impl SovereignGenerator {
    ...
    pub async fn package_cluster(
        &self,
        cluster_name: &str,
        files: Vec<PathBuf>,
        vsh: &VectorSpaceHeap,
    ) -> SovereignResult<GeneratedAsset> {
    ...
    pub async fn transmute_to_asset(
        &self,
        gem: &AuditFinding,
        vsh: &VectorSpaceHeap,
        ...
        let logic_payload = fs::read_to_string(&gem.files[0])
        ...
        fs::write(src_path.join("main.rs"), saas_code)
            .map_err(|e| SovereignError::IoError(e.to_string()))?;
        ...
        vsh.allocate(format!("MM_SAAS:{}", gem.title), vec![1.0; 128]);

        println!(
            "✨ ASSET GENERATED: {} | ESTIMATED EQUITY: ${:.2}",
...
"#,
        ...
        fs::write(path.join("Cargo.toml"), toml)
            .map_err(|e| SovereignError::IoError(e.to_string()))?;
        Ok(())
    }
}
